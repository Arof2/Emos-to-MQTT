...
char MQTT_Server[20]                  = "192.168.0.123";    // set default value
...
#define HOST_NAME                     "mqtt2can_gateway1"
...
void Init_MCP()
{
  ...
  CAN.onReceive(ProcessCANMessage);
  
  /*
     set mask, set filter
  */
  //CAN.filter(0, 0x00);        // accept ALL messages with standard identifier
  //CAN.filterExtended(0, 0x00);  // accept ALL messages with extended identifier
  ...
  */
  ...
  else
  {
    while (l_ptr != NULL)
    {
      ...
      Serial.print(l_ptr);
      Serial.print(" ");             
      l_ptr = strtok(NULL, MQTT_TX_PAYLOAD_SPLITTER);
      if (l_ptr != NULL)
      {
        l_DLC++;
        if (l_DLC > 8)
...
void MCP2515_ISR()
{
  ...
  ProcessCANMessage(CAN.parsePacket());
}

void ProcessCANMessage(int packetSize)
{
  long unsigned int l_CAN_ID  = CAN.packetId();
  uint8_t l_Ext_ID            = CAN.packetExtended(); 
  uint8_t l_DLC               = CAN.packetDlc();
  uint8_t l_RTR               = CAN.packetRtr();
  uint8_t l_RX_Buffer[8] = {0};
  ...
  { 
    ...
    while (CAN.available())
    {
      l_RX_Buffer[l_counter] = CAN.read();
      if(++l_counter >= l_DLC)
        break;
    }
